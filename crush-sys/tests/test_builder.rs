...
fn test_crush_create() {
    ...
    assert_that!(m.allowed_bucket_algs(), eq(
        (1 << crush_sys::crush_algorithm_CRUSH_BUCKET_UNIFORM) |
...
fn test_crush_make_bucket() {
    ...
    for alg in [BucketAlgorithm::Uniform, BucketAlgorithm::List, BucketAlgorithm::Straw2] {
...
fn test_crush_add_bucket() {
    ...
    let b = m.make_bucket(BucketAlgorithm::Straw2, hash, type_, &[], &[]).unwrap();
    let bucketno = m.add_bucket(0, b).unwrap();
    assert_that!(bucketno, eq(-1));
    
    // Try to add a bucket with the same ID again (should fail)
    let b2 = m.make_bucket(BucketAlgorithm::Straw2, hash, type_, &[], &[]).unwrap();
...
fn test_crush_bucket_add_item_uniform() {
    ...
    let b = m.make_bucket(BucketAlgorithm::Uniform, hash, type_, &[], &[]).unwrap();
    ...
    let mut b2 = m.make_bucket(BucketAlgorithm::Uniform, hash, type_, &[], &[]).unwrap();
...
fn test_crush_make_choose_args() {
    ...
    let root = m.make_bucket(BucketAlgorithm::Straw2, hash, type_, &[], &[]).unwrap();
    ...
    let b1 = m.make_bucket(BucketAlgorithm::Straw2, hash, type_, &items, &weights).unwrap();
    ...
    let b2 = m.make_bucket(BucketAlgorithm::Straw2, hash, type_, &items2, &weights2).unwrap();
