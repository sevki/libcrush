...
fn test_crush_do_rule_choose_arg() {
    ...
    // Create root bucket
    let root = m.make_bucket(BucketAlgorithm::Straw2, 0, root_type, &[], &[]).unwrap();
    ...
    for host in 0..host_count {
        ...
        let b = m.make_bucket(BucketAlgorithm::Straw2, 0, host_type, &items, &weights).unwrap();
        let bno = m.add_bucket(0, b).unwrap();
        host_buckets.push(bno);
        println!("host {} add bucket {} weight 0x{:x}", host, bno, weights.iter().sum::<i32>());
    ...
    for ruleno in &ruleno_list {
        ...
        // Test 1: Default mapping
        {
            ...
            let result_len = m.do_rule(*ruleno, value, &mut result, &weights, Some(&choose_args)).unwrap();
            ...
            let result_len = m.do_rule(*ruleno, value, &mut result, &weights, None).unwrap();
            assert_that!(result_len, eq(replication_count));
            assert_that!(result[0], eq(19));
            assert_that!(result[1], eq(13));
            
            // Swap weights in first position
            choose_args.swap_weights(host_9, 0, device_18, device_19).unwrap();
        ...
        // Test 2: Modifying second position weights
        {
            ...
            choose_args.swap_weights(host_6, 0, device_12, device_13).unwrap();
            ...
            choose_args.swap_weights(host_6, 1, device_12, device_13).unwrap();
    ...
    // Test ID remapping
    {
        ...
        let result_len = m.do_rule(ruleno_list[0], value, &mut result, &weights, Some(&choose_args)).unwrap();
        assert_that!(result_len, eq(replication_count));
        assert_that!(result[0], eq(19));
