use crate::{
    ...
    CrushBucket, // Removed CrushRule, CrushRuleStep
    CrushBucketList,
    CrushBucketStraw2,
    CrushBucketUniform,
    CrushMap,
...
use std::collections::HashSet; // Keep this as it's the standard way for the trait to be in scope.
...
fn crush_bucket_choose_uniform(bucket: &CrushBucketUniform, x: i32, r: i32) -> Option<i32> {
    ...
    let item_index = (h % bucket.common.size) as usize;

    bucket.common.items.get(item_index).copied()
}

fn crush_bucket_choose_list(bucket: &CrushBucketList, x: i32, r: i32) -> Option<i32> {
    if bucket.common.items.is_empty() || bucket.common.size == 0 {
        return None;
    }
    ...
    for i in 0..bucket.common.size as usize {
        if i >= bucket.item_weights.len()
            || i >= bucket.sum_weights.len()
            || i >= bucket.common.items.len()
        {
            // Data inconsistency
            return None;
        }
        ...
        let s = total_weight_for_bucket.saturating_sub(item_weight);

        if s == 0 {
            // If current item is the only one with weight, or all weights are zero.
            ...
            if i == (bucket.common.size as usize) - 1 {
                return Some(item_id);
            }
        ...
        if item_weight == 0 {
            continue;
        } // Cannot be chosen if weight is 0 with this formula
    ...
    if !bucket.common.items.is_empty() {
        ...
        let fallback_idx =
            (crush_hash32_rjenkins1(x as u32, bucket.common.id as u32, current_r as u32)
                % bucket.common.size) as usize;
...
fn crush_bucket_choose_straw2(bucket: &CrushBucketStraw2, x: i32, r: i32) -> Option<i32> {
    if bucket.common.items.is_empty() || bucket.common.size == 0 {
        return None;
    }

    let mut high_draw: i64 = i64::MIN;
    ...
    for i in 0..bucket.common.size as usize {
        ...
        // Compute hash - the C code uses crush_hash32_3 but we only have rjenkins1
        let h = crush_hash32_rjenkins1(x as u32, item_id as u32, r as u32);
        let u = (h & 0xffff) as u64;

        let draw = if item_weight > 0 {
            // The C implementation uses a lookup table for ln values
            // For now, we'll use a simplified calculation
            // The C code: ln = crush_ln(u) - 0x1000000000000ll;
            // where crush_ln maps [0,0xffff] to a fixed point ln value
            
            // Approximate the logarithm calculation
            // u is in range [0, 0xffff], we need ln((u+1)/0x10000)
            let u_normalized = (u + 1) as f64 / 65536.0;
            let ln_float = u_normalized.ln();
            
            // Convert to fixed point (multiply by 2^44 as in C implementation)
            let ln_fixed = (ln_float * (1i64 << 44) as f64) as i64;
            
            // Divide by weight (16.16 fixed point)
            // The C code does: draw = div64_s64(ln, weights[i]);
            if item_weight as i64 != 0 {
                ln_fixed / (item_weight as i64)
            } else {
                i64::MIN
            }
        } else {
        ...
        if high.is_none() || draw > high_draw {
            high = Some(i);
            high_draw = draw;
        }
    }

    high.map(|idx| bucket.common.items[idx])
}
...
fn crush_select_leaf(
    ...
    if let Some(bucket_obj) = get_bucket_by_id(map, current_bucket_id) {
        ...
        if target_bucket_type_filter != 0
            && bucket_obj.common().r#type as i32 != target_bucket_type_filter
        {
        ...
        if let Some(chosen_from_sub_bucket) = dispatch_bucket_choose(bucket_obj, x, r_attempt) {
            ...
            crush_select_leaf(
                map,
                chosen_from_sub_bucket,
                x,
                r_attempt + 1,
                local_reject_list,
                target_bucket_type_filter,
            )
        } else {
...
pub fn crush_do_rule(
    ...
    if rule_no < 0 || rule_no as usize >= map.rules.len() {
        eprintln!(
            "Rule {} not found or invalid (max rules {})",
            rule_no,
            map.rules.len()
        );
        return 0;
    ...
    if rule.steps.is_empty() && rule.len > 0 {
        eprintln!(
            "Rule {} has len {} but no steps defined.",
            rule_no, rule.len
        );
        return 0;
    }
    ...
    for step_idx in 0..num_steps {
        ...
        match CrushOpcode::from_u32(step.op) {
            ...
            Some(CrushOpcode::Emit) => {
                println!(
                    "Executing Emit. Current selection: {:?}, result space: {}/{}",
                    current_selection,
                    result.len(),
                    result_max
                );
            ...
            Some(op @ CrushOpcode::ChooseFirstN) | Some(op @ CrushOpcode::ChooseLeafFirstN) => {
                    ...
                    if let Some(bucket_obj) = get_bucket_by_id(map, *input_bucket_id) {
                        ...
                        if let Some(init_item) = initial_chosen {
                            if op == CrushOpcode::ChooseLeafFirstN {
                                let reject_list_for_leaf: HashSet<i32> =
                                    result.iter().copied().collect();
                                if let Some(leaf_item) = crush_select_leaf(
                                    map,
                                    init_item,
                                    _x,
                                    r_val,
                                    &reject_list_for_leaf,
                                    choose_type,
                                ) {
                                    if !reject_list_for_leaf.contains(&leaf_item) {
                                        new_selection.push(leaf_item);
                                    }
                                }
                            } else {
                                if !result.contains(&init_item) {
            ...
            Some(op @ CrushOpcode::ChooseIndep) | Some(op @ CrushOpcode::ChooseLeafIndep) => {
                let op_name = match op {
                ...
                let num_items_to_select_for_rule = if step.arg1 == 0 {
                ...
                if current_selection.is_empty()
                    && (op == CrushOpcode::ChooseIndep
                        || (op == CrushOpcode::ChooseLeafIndep && target_type == 0))
                {
                ...
                for i in 0..num_items_to_select_for_rule {
                    if result.len() + next_selection.len() >= result_max {
                        break;
                    }

                    let r_for_this_item_slot = i as i32;
                    let mut item_found_for_this_slot = false;

                    let retries_for_slot = map
                        .choose_local_tries
                        .saturating_add(map.choose_local_fallback_tries)
                        .max(1);

                    for try_num in 0..retries_for_slot {
                        let inputs_to_iterate =
                            if current_selection.is_empty() && op == CrushOpcode::ChooseLeafIndep {
                                if target_type != 0 {
                                    map.buckets
                                        .iter()
                                        .enumerate()
                                        .find(|(_idx, b_opt)| {
                                            b_opt.as_ref().map_or(false, |b| {
                                                b.common().r#type as i32 == target_type
                                            })
                                        })
                                        .map_or(vec![], |(idx, _)| vec![-1 - idx as i32])
                                } else {
                                    vec![]
                                }
                            } else {
                        ...
                        for &input_item_id in &inputs_to_iterate {
                            let factor =
                                try_num as i32 * (num_items_to_select_for_rule.max(1) as i32);
                            ...
                            let initial_chosen_item = if op == CrushOpcode::ChooseLeafIndep {
                                ...
                                if target_type != 0
                                    && bucket_obj.common().r#type as i32 != target_type
                                {
                                    continue;
                                }
                            ...
                            if let Some(selected_id) = final_item_to_add {
                                if !local_reject_list.contains(&selected_id) {
                                    ...
                                    local_reject_list.insert(selected_id);
                ...
                println!(
                    "{} completed. New current_selection: {:?} (size: {})",
                    op_name,
                    current_selection,
            ...
            Some(CrushOpcode::SetChooseTries) => {
                println!(
                    "Executing SetChooseTries (op: {}, val: {})",
                    step.op, step.arg1
                );
            }
            Some(CrushOpcode::SetChooseLeafTries) => {
                println!(
                    "Executing SetChooseLeafTries (op: {}, val: {})",
                    step.op, step.arg1
                );
            }
            Some(CrushOpcode::SetChooseLocalTries) => {
                println!(
                    "Executing SetChooseLocalTries (op: {}, val: {})",
                    step.op, step.arg1
                );
            }
            Some(CrushOpcode::SetChooseLocalFallbackTries) => {
                println!(
                    "Executing SetChooseLocalFallbackTries (op: {}, val: {})",
                    step.op, step.arg1
                );
            }
            Some(CrushOpcode::SetChooseLeafVaryR) => {
                println!(
                    "Executing SetChooseLeafVaryR (op: {}, val: {})",
                    step.op, step.arg1
                );
            }
            Some(CrushOpcode::SetChooseLeafStable) => {
                println!(
                    "Executing SetChooseLeafStable (op: {}, val: {})",
                    step.op, step.arg1
                );
            }
            None => {
                eprintln!(
                    "Unknown or unhandled CRUSH opcode: {}. Step details: {:?}",
                    step.op, step
                );
            }
        }
